...
import {
    addBlock, addBlockAtOrder, addCountedStroke, applyScorecardTee, blockLabel, clearHoleBlockOrderOverride, clearPlayerOrderOverride, createMulticamGroup, createProject, deleteBlock, duplicateBlock,
    effectiveHoleBlockOrder, effectivePlayerOrder, hasHoleBlockOrderOverride, hasPlayerOrderOverride, markSequenceReviewed, moveBlock, moveBlockInHoleOrder, movePlayerInOrder, moveSequence,
...
function playbookRoundTitle(round: number | null, types: BlockType[], par: number): string {
    if (round === null) return 'Zwischenszenen & Story';
    if (round === 1) return 'Abschlag';
    if (types.every((type) => type === 'putt')) return 'Auf dem Grün';
    if (types.some((type) => type === 'greenside' || type === 'bunker')) return 'Scoring Zone & Grünumfeld';
    if (round > par) return 'Zusätzliche Schläge';
    if (round === par) return 'Grün & Abschluss';
    return 'Spielbahn & Approach';
}
...
function RoundBuilder({ project, setProject, onOpenSequence, onAddClip }: { project: GolfProject; setProject: (project: GolfProject) => void; onOpenSequence: (sequenceId: string) => void; onAddClip: (mediaId: string, blockId: string) => void }) {
    ...
    const [orderAddPlayerId, setOrderAddPlayerId] = useState(project.settings.players[0]?.id ?? '');
    const [orderAddType, setOrderAddType] = useState<BlockType>('approach');
    const [orderAddRound, setOrderAddRound] = useState(2);
    ...
    const maxShotRound = Math.max(1, ...holeBlocks.map((block) => strokeNumberForBlock(project, block.id) ?? 0));
    ...
    return <section className="builder-workspace">
        ...
        {builderView === 'order' && <section className="true-order-board"><header><div><ListOrdered size={18} /><span><b>Par-{holePar}-Playbook · Loch {hole}</b><small>Schlagrunden statt Kartenwand · innerhalb jeder Runde gilt die tatsächliche Spielerfolge.</small></span></div>{hasHoleBlockOrderOverride(project, hole) && <button className="reset-player-order" onClick={() => setProject(clearHoleBlockOrderOverride(project, hole))}><RotateCcw size={12} /> Automatik wiederherstellen</button>}</header><div className="true-order-flow">{orderedBlocks.flatMap((block, index) => {
            const player = project.settings.players.find((item) => item.id === block.playerId);
            const sequences = block.sequenceIds.map((id) => project.sequences.find((sequence) => sequence.id === id)).filter(Boolean) as VirtualSequence[];
            ...
            const previousShotNumber = index ? strokeNumberForBlock(project, orderedBlocks[index - 1].id) : undefined;
            const roundBlocks = orderedBlocks.filter((item) => strokeNumberForBlock(project, item.id) === shotNumber);
            const roundIndex = roundBlocks.findIndex((item) => item.id === block.id);
            const roundTypes = [...new Set(roundBlocks.map((item) => item.type))];
            const roundHeader = previousShotNumber !== shotNumber ? <div className={`playbook-round-heading ${shotNumber && shotNumber > holePar ? 'over-par' : ''}`} key={`round-${shotNumber ?? 'story'}-${index}`}><span>{shotNumber ? String(shotNumber).padStart(2, '0') : 'B'}</span><div><b>{playbookRoundTitle(shotNumber, roundTypes, holePar)}</b><small>{shotNumber ? `Schlagrunde ${shotNumber} · ${shotNumber <= holePar ? `Par-Plan ${shotNumber}/${holePar}` : `${shotNumber - holePar} über Par-Plan`}` : 'Erzählmaterial ohne Schlagzählung'}</small></div><em>{roundBlocks.length} Karten</em></div> : null;
            const card = <article draggable className={`true-order-card ${draggedBlockId === block.id ? 'dragging' : ''} ${dragOverBlockId === block.id ? 'drag-over' : ''} ${sequences.length ? 'filled' : countsWithoutVideo ? 'count-only' : 'empty'}`} key={block.id} onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', block.id); setDraggedBlockId(block.id); }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; setDragOverBlockId(block.id); }} onDragLeave={() => setDragOverBlockId((current) => current === block.id ? undefined : current)} onDrop={(event) => { event.preventDefault(); if (draggedBlockId && roundBlocks.some((item) => item.id === draggedBlockId)) setProject(moveBlockInHoleOrder(project, hole, draggedBlockId, block.id)); setDraggedBlockId(undefined); setDragOverBlockId(undefined); }} onDragEnd={() => { setDraggedBlockId(undefined); setDragOverBlockId(undefined); }}>
                ...
                <footer><button disabled={roundIndex === 0} title="Innerhalb der Schlagrunde früher" onClick={() => setProject(moveBlockInHoleOrder(project, hole, block.id, roundBlocks[roundIndex - 1].id))}><ArrowLeft size={13} /></button><button disabled={roundIndex === roundBlocks.length - 1} title="Innerhalb der Schlagrunde später" onClick={() => setProject(moveBlockInHoleOrder(project, hole, block.id, roundBlocks[roundIndex + 1].id))}><ArrowRight size={13} /></button><span>{sequences.length ? 'IM FILM' : countsWithoutVideo ? 'ZÄHLT' : 'GEPLANT'}</span><button className="add-clip-action" title="Clip hinzufügen" onClick={() => setClipBrowserBlockId(block.id)}><Plus size={13} /></button><button title="Schlagdetails bearbeiten" onClick={() => setEditingBlockId(block.id)}><Pencil size={13} /></button>{sequences[0] && <button title="Aufnahme öffnen" onClick={() => onOpenSequence(sequences[0].id)}><Play size={13} /></button>}</footer>
            </article>;
            return roundHeader ? [roundHeader, card] : [card];
        })}</div><div className="playbook-add-shot"><div><Plus size={18} /><span><b>Weiteren Schlag einfügen</b><small>Zum Beispiel ein zusätzlicher Approach, Bunkerschlag oder Putt.</small></span></div><label><span>Spieler</span><select value={orderAddPlayerId} onChange={(event) => setOrderAddPlayerId(event.target.value)}>{project.settings.players.map((player) => <option value={player.id} key={player.id}>{player.name}</option>)}</select></label><label><span>Schlag</span><select value={orderAddType} onChange={(event) => setOrderAddType(event.target.value as BlockType)}>{BLOCK_TYPES.filter(([type]) => ['approach', 'greenside', 'bunker', 'putt', 'extra-shot', 'penalty'].includes(type)).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label><span>Als Schlag</span><select value={Math.min(orderAddRound, maxShotRound + 1)} onChange={(event) => setOrderAddRound(Number(event.target.value))}>{Array.from({ length: maxShotRound + 1 }, (_, roundIndex) => <option value={roundIndex + 1} key={roundIndex}>Schlag {roundIndex + 1}</option>)}</select></label><button onClick={() => { setProject(addBlockAtOrder(project, hole, orderAddPlayerId, orderAddType, Math.min(orderAddRound, maxShotRound + 1) - 1)); setOrderAddRound((current) => Math.min(maxShotRound + 2, current + 1)); }}><Plus size={14} /> Schlag hinzufügen</button></div><div className="true-order-hint"><GripVertical size={14} /><span><b>Drag & Drop:</b> Karten lassen sich innerhalb ihrer Schlagrunde sortieren. Die Playbook-Reihenfolge steuert Vorschau und Export.</span></div></section>}
        {builderView === 'players' && <>
